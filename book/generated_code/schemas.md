...
The `Float` type in Elm is a 64 bit floating point number.
Hence every `number` type is safely generated as a `Float`.
The `Int` type however uses less than 64 bits.
Currently, the generator ignores this fact and generates all `integer`s as an `Int`.
...
OpenAPI allows you to specify types using circular references.
Elm cannot handle [recursive type aliases](https://elm-lang.org/0.19.0/recursive-alias).
Therefore, when generating recursive types, additional custom types are generated that wrap the recursive properties in new types.
